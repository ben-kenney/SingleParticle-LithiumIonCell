...
def returnDs(y, material):
	...
	if (material == 'NMC'):
		...
		return (numpy.power(10,-63.061*numpy.power(y,5) + 239.8*numpy.power(y,4) - 343.74*numpy.power(y,3) + 232.45*numpy.power(y,2) - 74.337*y - 0.2517))/100/100
#-----------------------------------------------	
...
def Eref(material,soc,T,fudge=0):
	...
	def NMC(soc):
		x = numpy.array([3.125766183885334, -5.763342952859494, 2.124454409988303, 0.51324615231389, -1.999566459156232, 0.992457700580878, 0.007158221269832])
		return (x[0]*numpy.power(soc,3)+x[1]*numpy.power(soc,2)+x[2]*soc+x[3])/(numpy.power(soc,3)+x[4]*numpy.power(soc,2)+x[5]*soc+x[6])
	def NCA(soc):
		x = numpy.array([-47.09304396,    -2.64754588,   -71.38533023,  -800.63158077,  -340.36135014, 1284.86452671,  -429.83691774,   204.34231731,   233.74192887])
		return (x[0]*numpy.power(soc,5) + x[1]*numpy.power(soc,4) + x[2]*numpy.power(soc,3) + x[3]*numpy.power(soc,2) + x[4]*soc + x[5]) / (numpy.power(soc,3) + x[6]*numpy.power(soc,2) + x[7]*soc + x[8])
	def LCO(soc):
		if (soc < 0.43):
...
def ionicConductivity(ce, T, brugg, porosity):
    ...
    sigma = 0.0001*ce*numpy.power(-10.5+0.000668*ce+0.000000494*ce*ce+0.074*T-0.0000178*T*ce-0.000000000886*ce*ce*T-0.0000696*T*T+0.000000028*T*T*ce,2);
...
class electrode:
	...
	def __init__(self, parameters, electrodeType, cellNumber, writeData, T):
		...
		self.mass= self.volume*(density(self.activeMaterialType)*self.volFracSolids(self.activeMaterialType)*(1.0-self.porosity) + density('carbon')*self.volFracSolids('carbon')*(1.0-self.porosity) + density('PVDF')*self.volFracSolids('PVDF')*(1.0-self.porosity) + density('electrolyte')*self.porosity)
	...
	def volFracSolids(self, species):
		...
		totVolume = self.massFracAM/density(self.activeMaterialType) + self.massFracCarbon/density('carbon') + self.massFracBinder/density('PVDF')
		numerator = {
	...
	def porosity(self):
		...
		return 1.0-(self.apparentDensity*1000.0)/(self.volFracSolids(self.activeMaterialType)*density(self.activeMaterialType) + self.volFracSolids('carbon')*density('carbon') + self.volFracSolids('PVDF')*density('PVDF'))
	...
	def finiteDifference(self, dt, Iapp, totTime, T):				
		...
		if (totTime > 0):
			...
			D1[0,1]=2;D1[-1,-2]=2;
			x1=numpy.zeros([N]);x1[2:]=1/x[1:-1]
			x2=numpy.zeros([N]);x2[:N-2]=-1/x[1:-1]
...
class singleCell:
	...
	def calcCapacity(self):
		...
		if (self.schedule.Iapp['present'] < 0):
			...
			self.energy['cumulative_discharge']+= ((-1.0*Iapp*V) + (-1.0*Iapp_old*V_old))/2.0 * self.schedule.dt
	...
	def calcTambient(self, TcoolantIn, flowRate):
		...
		self.TcoolantOut = (self.Qheat*self.totVolume + m*coolantHeatCapacity*(TcoolantIn)) / (m*coolantHeatCapacity) #Tout in K
	...
	def Rohm(self,Iapp):
		...
		'''
		return self.cathode.ohmicResistance(Iapp, self.T, self.ce, 1.5)/arrhenius(self.electrolyteFactor['A'],self.electrolyteFactor['Ea'],self.T) + self.anode.ohmicResistance(Iapp, self.T, self.ce, 1.5)/arrhenius(self.electrolyteFactor['A'],self.electrolyteFactor['Ea'],self.T) + self.sep.ohmicResistance(Iapp, self.T, self.ce, 1.5)/arrhenius(self.electrolyteFactor['A'],self.electrolyteFactor['Ea'],self.T)
...
class cycleSchedule:
	...
	def checkStopCondition(self, voltage):
		...
		if (self.schedule[self.step][2] == 0 and ( (self.Iapp['present'] < 0 and voltage <= self.schedule[self.step][3])  or (self.Iapp['present'] > 0 and voltage >= self.schedule[self.step][3]))):
